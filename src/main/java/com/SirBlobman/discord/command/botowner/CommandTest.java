package com.SirBlobman.discord.command.botowner;

import com.SirBlobman.discord.command.ICommand;
import com.SirBlobman.discord.command.ICommand.BotOwnerOnly;
...
public class CommandTest extends ICommand {
    private static final String[] aliases = {"testing", "testthebot", "probando"};
    public CommandTest() {super("test", "(SirBlobman Only) Test command for the bot", "<test>", aliases);}
    ...
    private void sendOperatingSystemInfo(TextChannel channel) {
        ...
        String osImageName = "/assets/icons/os/" + (osName.contains("Windows") ? "windows" : (osName.contains("Mac OS") ? "apple" : "linux")) + ".png";
