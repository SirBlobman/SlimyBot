...
public final class ImageUtil extends Util {
    public static BufferedImage fromJar(String name) {
        ...
        try {
            ...
            log("Loaded image from jar with name '" + name + "' and size '" + imageWidth + "x" + imageHeight + "'.");
            return image;
        } catch(IOException ex) {
            log("Failed to get image from jar '" + name + "'.");
            ex.printStackTrace();
            return null;
        }
    }
    ...
    public static InputStream toInputStream(BufferedImage original, String imageType) {
        try {
            ...
            log("Failed to create input stream from image with type '" + imageType + "'");
            ex.printStackTrace();
            return null;
        }
    }
